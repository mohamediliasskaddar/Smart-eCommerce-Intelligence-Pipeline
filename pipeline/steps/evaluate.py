...
if xgb:
    ...
    grade = "EXCELLENT" if xgb["roc_auc"] >= 0.90 else \
            "GOOD"      if xgb["roc_auc"] >= 0.80 else \
            "ACCEPTABLE" if xgb["roc_auc"] >= 0.70 else "NEEDS WORK"
...
if clust:
    ...
    sil_grade = "WELL SEPARATED" if sil >= 0.5 else \
                "ACCEPTABLE"     if sil >= 0.3 else "OVERLAPPING"
    print(f"  Grade : {sil_grade}  (≥0.5 = well separated clusters)")

    print(f"\n  DBSCAN Anomalies : {db['n_anomalies']} products ({db['anomaly_pct']}%)")
    anom_note = "normal" if db["anomaly_pct"] < 5 else \
                "check anomalies — may indicate data quality issues" if db["anomaly_pct"] > 15 \
                else "acceptable"
    ...
    report["clustering"] = {
        "silhouette": sil, "silhouette_grade": sil_grade,
        "anomalies": db["n_anomalies"], "anomaly_pct": db["anomaly_pct"],
...
if assoc:
    ...
    print(f"  Frequent itemsets : {assoc['frequent_itemsets']}")
    print(f"  Total rules       : {assoc['total_rules']}")
    print(f"  Rules → topk:1    : {assoc['topk_rules']}")

    rules_grade = "RICH"      if assoc["total_rules"] >= 50  else \
                  "MODERATE"  if assoc["total_rules"] >= 15  else \
                  "SPARSE"
